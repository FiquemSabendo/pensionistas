...
df_graph <- read_csv(
    "https://raw.githubusercontent.com/FiquemSabendo/pensionistas/master/202102-202001_Pensionistas_DEFESA_FBarbalhoApp.csv", 
...
h5("A aba Metodologia informa como citar a Fiquem Sabendo ao fazer referência aos dados dessa aplicação")
```
...
#cadastro<- data.table::fread(file="202102_Cadastro.csv")
#remuneracao<- data.table::fread(file="202102_Remuneracao.csv")
#varios_vinculos <- readRDS("varios_vinculos.RDS")
# 
# save(list = c("cadastro", "remuneracao"), file = "dados_pensionstas_militares.RData")

microdados <- readRDS("microdados.RDS")
...
renderDataTable({
  ...
  # remuneracao %>%
  #    mutate(`REMUNERAÇÃO BÁSICA BRUTA (R$)`= str_replace(`REMUNERAÇÃO BÁSICA BRUTA (R$)`,",","."),
  #              `REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`= str_replace(`REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`,",","."),
  #            `REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`=  as.numeric(`REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`)
  #           ) %>%
  #   filter(`REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`<0) %>%
  #   summarise(total_negativo= sum(`REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`),
  #             quantidade = n())
  ...
  #   summarise(
  #     quantidade_vinculos = n()
  #   ) %>%
  #   ungroup() %>%
  #   filter(quantidade_vinculos>1)
  
  #saveRDS(varios_vinculos, "varios_vinculos.RDS")
  ...
  #          !is.na(NOME_INSTITUIDOR_PENSAO)) %>%
  #   inner_join(
  #     remuneracao %>%
  #       mutate(`REMUNERAÇÃO BÁSICA BRUTA (R$)`= str_replace(`REMUNERAÇÃO BÁSICA BRUTA (R$)`,",","."),
  #              `REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`= str_replace(`REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`,",","."))%>%
  #       rename(`Remuneração bruta` = `REMUNERAÇÃO BÁSICA BRUTA (R$)`,
  #              `Remuneração líquida`= `REMUNERAÇÃO APÓS DEDUÇÕES OBRIGATÓRIAS (R$)`) %>%
  #       select(Id_SERVIDOR_PORTAL, `Remuneração bruta`, `Remuneração líquida`) 
  #   ) %>% 
  #   left_join(varios_vinculos) %>%
  #   mutate(`Vários vínculos` = ifelse(is.na(quantidade_vinculos),"Não", "Sim")) %>%
  #   arrange(NOME_INSTITUIDOR_PENSAO) %>%
  #   mutate(`Instituidor da Pensão` = factor(NOME_INSTITUIDOR_PENSAO),
  #          `Pensionsta` = factor(NOME),
  #          `Tipo de Beneficiário`= factor(TIPO_PENSAO), 
  #          `Orgão do instituidor` = factor(ORG_LOTACAO_INSTITUIDOR_PENSAO),
  #          `Cargo do instituidor` = factor(DESCRICAO_CARGO_INSTITUIDOR_PENSAO),
  #          `Remuneração bruta` =  as.numeric(`Remuneração bruta`),
  #          `Remuneração líquida`=  as.numeric(`Remuneração líquida`)
  #          ) %>%
  #   select(`Instituidor da Pensão`,
  #          `Pensionsta`,
  #          `Tipo de Beneficiário`,
  #          `Orgão do instituidor`,
  ...
  #          )
   ...
   #print("gerou microdados")
   
   #saveRDS(microdados, "microdados.RDS")
   ...
   DT::datatable(
    ...
    )%>% formatRound(6:7,digits = 2, mark=".", dec.mark= ",")
}
